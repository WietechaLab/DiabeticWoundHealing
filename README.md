# DiabeticWoundHealing
Code to reproduce the analyses and figures in the diabetic wound healing study of Wietecha, et al. 2026 <BR>

The code is split up into R scripts and annotated according to the figure numbers in manuscript: <BR>
https://www.biorxiv.org/content/10.64898/2026.04.26.720829v1.full <BR>

The RDS files of Seurat objects needed to reproduce the results can be found on Zenodo: <BR>
https://zenodo.org/records/20549537 (DOI: 10.5281/zenodo.20549537) <BR>

The single-cell sequencing raw data are uploaded to NCBI Sequence Read Archive (SRA): <BR>
BioProject number PRJNA1467292 <BR>
The single-cell sequencing processed datasets are uploaded to NCBI Gene Expression Omnibus (GEO): <BR>
Accession number GSE333591 (Reviewer token: kdafmyoknzappof)
